...
This folder contains the browser-controlled nbody demos so the normal `sw/programs/run.sh` workflow stays focused on building/running programs. It supports the original 2D `nbody` program and the newer `nbody-3d` program.
...
### Program/kernel (`sw/programs/nbody/nbody.c`)
...
### Loader/adapter (`sw/programs/nbody/fpga.py` and demo backend)
