...
The repository has a generic flow for compiling a program, loading it on the FPGA through UART, running it in chunks, dumping a program-defined output window, and optionally visualizing the results.  The root entry point is `./run.sh`; the program-specific behavior lives under `sw/programs/<program>/`.
...
- let `sw/programs/nbody/fpga.py` append the output to `sw/programs/nbody/data.csv` and update the live visualization,
...
This exists so users can start from the repository root while keeping all program build/run logic inside `sw/programs/`.

### Program runner: `sw/programs/run.sh`

`sw/programs/run.sh` performs these steps:

1. Discovers available program directories under `sw/programs/`.
...
3. Builds the selected program with:
   ...
   ```bash
   make -C sw/programs PROG=<program> clean
   make -C sw/programs PROG=<program> <target>
   ```
...
4. If `--fpga` was requested, it calls the generic Python FPGA runner:
   ...
   ```bash
   python3 sw/programs/fpga_run.py \
       --program <program> \
       --port <port> \
       --baud <baud> \
       --steps-per-run <steps> \
...
The shell script does not know the nbody memory layout.  It only builds the program and forwards FPGA-run options to `sw/programs/fpga_run.py`.

### Generic FPGA loop: `sw/programs/fpga_run.py`

`sw/programs/fpga_run.py` owns the common UART lifecycle.  It imports one program-specific adapter from:
...
1. Import `sw/programs/<program>/fpga.py` and construct `ProgramAdapter(program_dir=...)`.
...
5. Open the UART monitor using `sw/host/baremetal/gpgpu_uart.py`.
...
### Program-specific adapter: `sw/programs/nbody/fpga.py`

`sw/programs/nbody/fpga.py` defines the nbody host-side ABI.  It must match the DMEM constants used by `sw/programs/nbody/nbody.c`.
...
- `configure(...)`
  - creates or truncates `sw/programs/nbody/data.csv`,
...
- `process_output(...)`
  ...
  - appends one row to `sw/programs/nbody/data.csv`,
  - updates `sw/programs/nbody/fpga_latest.svg` for live preview.

- `finalize(visualize=True)`
  - runs `sw/programs/nbody/visualize.py` after the FPGA loop if visualization was requested.
...
1. Keep `sw/programs/nbody/fpga.py` and `sw/programs/nbody/nbody.c` in agreement on all DMEM word offsets.
