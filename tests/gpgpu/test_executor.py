...
class ProgramAdapterTests(unittest.TestCase):
    ...
    def require_riscv_tools(self):
        if shutil.which("make") is None:
            self.skipTest("make is required for RISC-V ELF compatibility adapter test")
        if shutil.which("riscv64-unknown-elf-gcc") is None and shutil.which("riscv-none-elf-gcc") is None:
            self.skipTest("riscv64-unknown-elf-gcc or riscv-none-elf-gcc is required for RISC-V ELF compatibility adapter test")
    ...
    def test_unimplemented_program_image_still_fails_clearly(self):
        stdout = io.StringIO()
        stderr = io.StringIO()
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
        ...
        self.assertEqual(code, 2)
        self.assertEqual(stdout.getvalue(), "")
        self.assertIn("no executor adapter registered for sw.program.image", stderr.getvalue())
